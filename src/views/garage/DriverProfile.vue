    ...
    <el-container>
      ...
      <el-main>
        <el-table :data="profileList" style="width:100%;color:black;" stripe>
          ...
          <el-table-column align="center" prop="licenseNo" label="驾驶证号" width="140" />
...
export default {
  ...
  methods: {
    ...
    save(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        if (this.dialog.item.id == null || this.dialog.item.id === "") {
          addDriverProfile(this.dialog.item).then((res) => {
            if (res.flag) {
          ...
          updateDriverProfile(this.dialog.item).then((res) => {
            if (res.flag) {
    ...
    getProfileList() {
      listDriverProfilePage(this.queryParams)
        .then((res) => {
